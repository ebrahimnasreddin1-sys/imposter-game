...
export default function FinalGuessScreen({ imposterName, options, secretChar, onGuess }) {
  ...
  const getCardStyle = (charObj) => {
    if (!revealed) return 'bg-white/[0.04] border-white/[0.08] active:scale-[0.97]';
    if (charObj.name === selected && charObj.name === secretChar.name)
      return 'bg-green-500/15 border-green-500/50 neon-green';
    if (charObj.name === selected && charObj.name !== secretChar.name)
      return 'bg-red-500/15 border-red-500/50 neon-red';
  ...
  const getGlow = (charObj) => {
    if (!revealed) return 'none';
    if (charObj.name === secretChar.name) return 'green';
    if (charObj.name === selected) return 'red';
    return 'none';
  };
        ...
        <div className="grid grid-cols-2 gap-2.5">
          {options.map((charObj, i) => (
            <button
              ...
              disabled={revealed}
              className={`p-3 rounded-2xl border transition-all flex flex-col items-center gap-2 ${getCardStyle(charObj)}`}
            ...
            >
              <CharacterImage
                ...
                image={charObj.image}
                size="md"
                glowColor={getGlow(charObj)}
              />
              <span className={`text-sm font-bold ${getNameColor(charObj)}`}>
                {charObj.name}
              </span>
            </button>
          ))}
